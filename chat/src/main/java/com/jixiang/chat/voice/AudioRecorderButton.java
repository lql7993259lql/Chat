...
public class AudioRecorderButton extends Button {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:// 手指按下
			try {
				...
				Toast.makeText(context, "录音授权失败", Toast.LENGTH_SHORT).show();
			}
			break;
	...
	private void changeState(int state) {
		if (mCurrentState != state) {
			...
			case STATE_NORMAL:
				try {
					setBackgroundResource(R.drawable.chat_btn_recorder_normal);
					setText(R.string.chat_str_recorder_normal);
				} catch (Exception e) {
				}
				break;
			...
			case STATE_RECORDING:
				try {
					setBackgroundResource(R.drawable.chat_btn_recorder_recording);
					setText(R.string.chat_str_recorder_recording);
					if (isRecording) {
