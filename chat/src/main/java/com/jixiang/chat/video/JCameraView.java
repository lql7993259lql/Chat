package com.jixiang.chat.video;

import java.io.IOException;

import com.jixiang.chat.R;
import com.jixiang.chat.video.listener.CaptureListener;
...
import com.jixiang.chat.video.listener.TypeListener;
import com.jixiang.chat.video.state.CameraMachine;
import com.jixiang.chat.video.util.FileUtil;
import com.jixiang.chat.video.util.LogUtil;
import com.jixiang.chat.video.util.ScreenUtils;
import com.jixiang.chat.video.view.CameraView;
...
/**
 * ===================================== 作 者: 王达 版 本：1.0.4 创建日期：2018/1/26 描 述：
 * =====================================
 */
public class JCameraView extends FrameLayout
		implements CameraInterface.CameraOpenOverCallback, SurfaceHolder.Callback, CameraView {
	...
	public JCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (!isInEditMode()) {
			...
			iconSize = a.getDimensionPixelSize(R.styleable.JCameraView_iconSize, (int) TypedValue
					.applyDimension(TypedValue.COMPLEX_UNIT_SP, 35, getResources().getDisplayMetrics()));
			iconMargin = a.getDimensionPixelSize(R.styleable.JCameraView_iconMargin, (int) TypedValue
					.applyDimension(TypedValue.COMPLEX_UNIT_SP, 15, getResources().getDisplayMetrics()));
			iconSrc = a.getResourceId(R.styleable.JCameraView_iconSrc, R.drawable.chat_ic_camera);
	...
	private void initView() {
		...
		View view = LayoutInflater.from(mContext).inflate(R.layout.chat_camera_view, this);
	...
	private void setFlashRes() {
		...
		case TYPE_FLASH_AUTO:
			mFlashLamp.setImageResource(R.drawable.chat_ic_flash_auto);
			machine.flash(Camera.Parameters.FLASH_MODE_AUTO);
			break;
		case TYPE_FLASH_ON:
			mFlashLamp.setImageResource(R.drawable.chat_ic_flash_on);
			machine.flash(Camera.Parameters.FLASH_MODE_ON);
			break;
		case TYPE_FLASH_OFF:
			mFlashLamp.setImageResource(R.drawable.chat_ic_flash_off);
